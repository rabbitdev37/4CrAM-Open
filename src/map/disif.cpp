...
int disif_parse_login(int fd) {
	if (RFIFOREST(fd) < 56)
		...
		if (runflag != MAPSERVER_ST_RUNNING ||
			...
			discord_server.ip = ip;
			discord_server.port = port;
...
* 0E03 <packet len>.W <channel name>.20B <user name>.24B <message>.?B
*/
...
int disif_send_message_to_disc(struct Channel *channel, char *msg) {
	...
	WFIFOW(discord_server.fd, 0) = 0xE04;
...
int disif_parse(int fd) {
	...
	if (cmd < 0x0E00 || cmd >= 0x0E00 + ARRAYLENGTH(dis_recv_packet_length) || dis_recv_packet_length[cmd - 0x0E00] == 0) {
	...
	while (RFIFOREST(fd) >= 2) {
		...
		// Check packet length
		if ((len = dis_check_length(fd, dis_recv_packet_length[cmd - 0x0E00])) == 0) {
			//invalid cmd, just close it
			ShowError("Unknown packet 0x%04x from discord server, disconnecting.\n", RFIFOW(fd, 0));
			set_eof(fd);
			return 0;
		...
		switch (RFIFOW(fd, 0)) {
		case 0x0e01: next = disif_parse_login(fd); return 0;
		case 0x0e03: next = disif_parse_message_from_disc(fd); break;
		default:
			ShowError("Unknown packet 0x%04x from discord server, disconnecting.\n", RFIFOW(fd, 0));
			set_eof(fd);
			return 0;
